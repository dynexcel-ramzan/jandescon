        ...
        for employee in employees:
            ...
            for shift_line in shift_lines:
                ...
                if attendance_hours < (current_shift[0].hours_per_day- 1.50) and attendance_hours > 0:
                    ...
                    nleave_exist = self.env['hr.leave'].sudo().search([('employee_id','=', employee.id),('request_date_from','<=', shift_line.date),('request_date_to','>=',  shift_line.date),('state','in',('validate','confirm'))], limit=1)
                    if nleave_exist:
                        if nleave_exist.state=='validate':
                            pass
                            sr_no -= 1
                        else:
                    	    absent_list.append({
